...
void loop0(void *parameter)
{
  ...
  switch (button){
    ...
    case '#': pressed_bttn = !pressed_bttn;  break; //Serial.print(pressed_bttn);
    ...
    if (not pressed_bttn){
      if (single_mode==0){
        ...
        if (row==1){
          ...
          {
            for (int i = 0; i < LEDS_COUNT; i=i+space+row-1){       //nie działa space i row coś się psuje, można dodać offset i sterowanie jedną diodą dla spaceu 60. np zamiast 11 przy if to 60 i wtedy jedna dioda 
             for (int j = 0; j <row; j++){strip.setLedColorData(i+j, color[col_nr][0], color[col_nr][1], color[col_nr][2]);}}    //rząd diód i przerwy
          }        
          strip.show();
          curr_t=millis();   
          delay(500/freq-curr_t+prev_t); 
          ...
          strip.show();
          curr_t=millis();
          delay(500/freq-curr_t+prev_t); //tutaj trzeba odjąć czas trwania fora za pomocą funkcji milis przed i po forze
      }
      else
      {
